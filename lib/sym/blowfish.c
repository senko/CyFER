...
static inline u32 F(BLOWFISH_CTX *ctx, u32 x)
{
	u8 a, b, c, d;
	a = (u8) (x >> 24); b = (u8) ((x >> 16) & 255); c = (u8) ((x >> 8) & 255); d = (u8) (x & 255);
	return ((ctx->sbox[0][a] + ctx->sbox[1][b]) ^ ctx->sbox[2][c]) + ctx->sbox[3][d];
}

static void blowfish_encrypt_block(BLOWFISH_CTX *ctx, u8 *block)
{
	int i;
	u32 xl, xr, t;

	xl = big_load32(block); xr = big_load32(block + 4);
	for (i = 0; i < 16; i++) {
		xl ^= ctx->P[i];
		xr ^= F(ctx, xl);
		t = xl; xl = xr; xr = t;	
	...
	xr ^= ctx->P[16];
	xl ^= ctx->P[17];
	big_store32(xl, block); big_store32(xr, block + 4);
}

static void blowfish_decrypt_block(BLOWFISH_CTX *ctx, u8 *block)
{
	int i;
	u32 xl, xr, t;
...
static void key_schedule(BLOWFISH_CTX *ctx, const unsigned char *key, size_t keylen)
{
	u8 K[18 * 4];
	u8 tmp[8];
	int i, j;

	for (i = 0; i < 4; i++) memcpy(ctx->sbox[i], S[i], 256 * 4);
	for (i = 0; i < 18 * 4; i++) K[i] = key[i % keylen];
	for (i = 0; i < 18; i++) ctx->P[i] = P[i] ^ big_load32(K + i * 4);

	memset(tmp, 0, 8);
	for (i = 0; i < 9; i++) {
		blowfish_encrypt_block(ctx, tmp);
		ctx->P[2 * i] = big_load32(tmp);
		ctx->P[2 * i + 1] = big_load32(tmp + 4);
	}

	for (i = 0; i < 4; i++) for (j = 0; j < 128; j++) {
		blowfish_encrypt_block(ctx, tmp);
		ctx->sbox[i][2 * j] = big_load32(tmp);
		ctx->sbox[i][2 * j + 1] = big_load32(tmp + 4);
	}
...
void CYFER_BLOWFISH_Init(BLOWFISH_CTX *ctx, const unsigned char *key, size_t keylen)
{
	key_schedule(ctx, key, keylen);
}

void CYFER_BLOWFISH_Finish(BLOWFISH_CTX *ctx)
{
	ctx = NULL; /* suppress warning */
}

void CYFER_BLOWFISH_Encrypt(BLOWFISH_CTX *ctx, const unsigned char *input, unsigned char *output)
{

	memcpy(output, input, 8);
	blowfish_encrypt_block(ctx, output);
}

void CYFER_BLOWFISH_Decrypt(BLOWFISH_CTX *ctx, const unsigned char *input, unsigned char *output)
{

	memcpy(output, input, 8);
	blowfish_decrypt_block(ctx, output);
}
